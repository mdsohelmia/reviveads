 ...
 *
 */
class MAX_Admin_Invocation_djax {
    ...
    function getAllowedVariables()
    {
        $aVariables = array(
			...
			//Dac015-JW player configurations*/
			'limit','adposition','dadposition','withoutplayer',	//DAC015-VAST            
            // Others
            'bannerUrl',
            'block',
            'blockcampaign',
...
/*DAC015*/
		 ...
		 $results=OA_Dal_Delivery_fetchAssoc(OA_Dal_Delivery_query("select * from ".$table_prefix."zones where zoneid='".$zoneid."'"));

		$allow_delivery=array(5,4,3,1,2,0);


            foreach($invocationTags as $pluginKey => $invocationTag) { 
		if($results['delivery']==6 && ($invocationTag->getName()=='VMAP Video Tag' || $invocationTag->getName()=='Inline VAST 4.0' || $invocationTag->getName()=='Inline VAST 4.1' || $invocationTag->getName()=='Inline VAST 3.0' || $invocationTag->getName()=='Inline VAST 2.0' || $invocationTag->getName()=='Inline VAST 1.0' || $invocationTag->getName()=='Inline Wrapper VAST 1.0' || $invocationTag->getName()=='Inline Wrapper VAST 2.0' || $invocationTag->getName()=='Inline Wrapper VAST 3.0' || $invocationTag->getName()=='Inline Wrapper VAST 4.0' || $invocationTag->getName()=='Inline Wrapper VAST 4.1' || $invocationTag->getName()=='VAST Ad Pod' || $invocationTag->getName()=='VAST Ad Pod 4.0' || $invocationTag->getName()=='VAST Ad Pod 4.1' || $invocationTag->getName()=='VAST Ad Pod Wrapper' || $invocationTag->getName()=='VAST Ad Pod Wrapper 4.0' || $invocationTag->getName()=='VAST Ad Pod Wrapper 4.1' || $invocationTag->getName()=='Audio VAST 4.1'))
		{
			 if ($invocationTag->isAllowed($extra, $this->server_same)) {
		            $aOrderedComponents[$invocationTag->getOrder()] =
		                array(
		...
		else if($results['delivery']==7 && ($invocationTag->getName()=='Overlay Vast 2.0' || $invocationTag->getName()=='Overlay Vast 3.0' || $invocationTag->getName()=='Overlay Vast 1.0' || $invocationTag->getName()=='Overlay Vast 4.0' || $invocationTag->getName()=='Overlay Vast 4.1' || $invocationTag->getName()=='Overlay Wrapper VAST 1.0' || $invocationTag->getName()=='Overlay Wrapper VAST 2.0' || $invocationTag->getName()=='Overlay Wrapper VAST 3.0' || $invocationTag->getName()=='Overlay Wrapper VAST 4.0' || $invocationTag->getName()=='Overlay Wrapper VAST 4.1'))
		{
			 if ($invocationTag->isAllowed($extra, $this->server_same)) {
		            $aOrderedComponents[$invocationTag->getOrder()] =
		                array(
		...
		else if(in_array($results['delivery'],$allow_delivery) && $invocationTag->getName()!='VMAP Video Tag' && $invocationTag->getName()!='Inline VAST 3.0' && $invocationTag->getName()!='Inline VAST 4.0' && $invocationTag->getName()!='Inline VAST 4.1' && $invocationTag->getName()!='Inline VAST 2.0' && $invocationTag->getName()!='Inline VAST 1.0' && $invocationTag->getName()!='Overlay Vast 2.0' && $invocationTag->getName()!='Overlay Vast 3.0'  && $invocationTag->getName()!='Overlay Vast 1.0'  && $invocationTag->getName()!='Overlay Vast 4.0'  && $invocationTag->getName()!='Overlay Vast 4.1'  && $invocationTag->getName()!='Inline Wrapper VAST 1.0' && $invocationTag->getName()!='Inline Wrapper VAST 2.0' && $invocationTag->getName()!='Inline Wrapper VAST 3.0' && $invocationTag->getName()!='Inline Wrapper VAST 4.0' && $invocationTag->getName()!='Inline Wrapper VAST 4.1' && $invocationTag->getName()!='Overlay Wrapper VAST 1.0' && $invocationTag->getName()!='Overlay Wrapper VAST 2.0' && $invocationTag->getName()!='Overlay Wrapper VAST 3.0' && $invocationTag->getName()!='Overlay Wrapper VAST 4.0' && $invocationTag->getName()!='Overlay Wrapper VAST 4.1' && $invocationTag->getName()!='VAST Ad Pod' && $invocationTag->getName()!='VAST Ad Pod 4.0' && $invocationTag->getName()!='VAST Ad Pod 4.1' && $invocationTag->getName()!='VAST Ad Pod Wrapper' && $invocationTag->getName()!='VAST Ad Pod Wrapper 4.0' && $invocationTag->getName()!='VAST Ad Pod Wrapper 4.1' && $invocationTag->getName()!='Audio VAST 4.1')
		{ 
			 if ($invocationTag->isAllowed($extra, $this->server_same)) 
			{
		            $aOrderedComponents[$invocationTag->getOrder()] =
...
//~ VideoAdsHelperDjax::displayWarningMessage("
//~ <b>Note:	Zone invocation codes are not used for Video Ads.</b> <br/>
		...
		//~ Please refer to the <a href='".VideoAdsHelperDjax::getHelpLinkVideoPlayerConfig()."' target='_blank'>Video Ads Plugin User Guide</a> for instructions.
		//~ ");
	$maxInvocation = new MAX_Admin_Invocation_djax();
