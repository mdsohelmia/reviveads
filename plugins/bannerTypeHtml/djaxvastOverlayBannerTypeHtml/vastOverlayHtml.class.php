 ...
 */
class Plugins_BannerTypeHTML_djaxvastOverlayBannerTypeHtml_vastOverlayHtml extends Plugins_BannerTypeHTML_djaxvastInlineBannerTypeHtml_vastBase
{
    ...
    {
        return 'Overlay Video Ad(VAST 2.0,VAST 3.0)';
    }
    ...
    function addVastOverlayFormatsRadioButton($form, $bannerRow, &$overlayFormatValue, &$overlayFormatJs )
    {
        ...
        $overlayFormatToHandler = array(
			...
			VAST_OVERLAY_FORMAT_WRAPPER => 'phpAds_formOverlayIswrapperMode();'				 
        );
        ...
        if ( getVideoOverlaySetting( 'isVastOverlayAsImageEnabled' ) ) 
	{
            $overlayFormats[] = $form->createElement('radio', 'vast_overlay_format', '',
                "Vast Wrapper Ads",
                VAST_OVERLAY_FORMAT_WRAPPER, array('id' => 'vast-overlay-format-wrapper',
                    'onClick' => $overlayFormatToHandler[VAST_OVERLAY_FORMAT_WRAPPER] ));
	...
	/*DAC015*/
        ...
        $overlayFormatJs = <<<OVERLAY_FORMAT_JS
            ...
            function phpAds_formOverlayIswrapperMode()
	    {
		...
		$("#div-overlay-format-wrapper").show('slow');
		...
		$("#div-overlay-format-wrapper").hide('fast');
...
OVERLAY_OPTION_JS;
      ...
      /*DAC015-hIDE BEHALF OF VAST UPGRADE*/
	 /* $overlayClickActions[] = $form->createElement('radio', 'vast_overlay_action', '',
            "Play a video",
    ...
    function addVastOverlayAsWrapper($form, $bannerRow)
    {
        $form->addElement('header', 'overlay_wrapper_header', "Overlay Wrapper Ads");

	$form->addDecorator ( 'overlay_wrapper_header', 'tag', array ( 'tag' => 'div', 'attributes' => array ('id' => 'div-overlay-format-wrapper') ) );

	/*DAC015*/

	 $form->addElement('text', 'vast_overlay_wrapper', 'Third Party Vast Tag');

	 //~ $getvastversion=getvastversion();

	 //~ $form->addElement('select', 'vast_overlay_version','Vast Version', $getvastversion);
	/*DAC015*/
    }
	...
	$this->addVastOverlayAsWrapper($form, $bannerRow);/*DAC015*/
	...
	/*To update overlay ads using banner vast element id*/

	$form->addElement('hidden', 'banner_vast_element_id', "banner_vast_element_id");/*DAC015*/
        $form->addElement('hidden', 'vast_element_type', "singlerow");/*DAC015*/

	/*To update overlay ads using banner vast element id*/

        $this->addThirdPartyImpressionTracking($form);
	$this->addoverlayadvanced($form);
	
	$this->addThirdPartyClickTracking($form);
